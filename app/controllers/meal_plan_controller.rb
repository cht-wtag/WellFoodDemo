...
@meal_plan.build_meals
 ...
 @meal_plan=current_user.meal_plans.build(meal_plan_params)
    if @meal_plan.save
      redirect_to meal_plan_path(@meal_plan), notice: "you have created a meal plan"
    else
  ...
  def update
    ...
    if @meal_plan.update_attributes(meal_plan_params)
      redirect_to meal_plan_path(@meal_plan), notice: "you have edited a meal plan"
    else
      puts @meal_plan.errors.inspect
      @errors=@meal_plan.errors.full_messages
  end
 ...
 @meal_plan=current_user.meal_plans.find(params[:id])
  ...
  def meal_plan_params
    params.require(:meal_plan).permit(
                                  :start_date,
                                  :end_date,
                                   meals_attributes: [:id, :date, :item_id])
  end
